...
const AuthorizationPage = () => {
  ...
  const validateForm = () => { const newErrors = {}; if (!formData.email.trim()) newErrors.email = 'Email обязателен'; else if (!/\S+@\S+\.\S+/.test(formData.email)) newErrors.email = 'Неверный формат email'; if (!formData.password) newErrors.password = 'Пароль обязателен'; setErrors(newErrors); return Object.keys(newErrors).length === 0; };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (docSnap.exists()) {
        ...
        console.warn("Документ пользователя не найден в Firestore для uid:", firebaseUser.uid, ". Создание базового профиля.");
        ...
        console.log("Базовый профиль создан в Firestore для нового пользователя (AuthPage):", userProfileData);
      }
      ...
      console.log('ОБЪЕКТ ДЛЯ LOCALSTORAGE ПРИ ВХОДЕ (AuthPage - из Firestore):', userToStore);
      localStorage.setItem('user', JSON.stringify(userToStore));

      navigate('/profile');
    } catch (error) {

      console.error('Ошибка авторизации (AuthPage):', error.code, error.message);
      let firebaseErrorMessage = 'Ошибка входа.';
      switch (error.code) {
        case 'auth/user-not-found': case 'auth/wrong-password': case 'auth/invalid-credential': firebaseErrorMessage = 'Неверный email или пароль.'; break;
        case 'auth/invalid-email': firebaseErrorMessage = 'Неверный формат email.'; break;
        case 'auth/user-disabled': firebaseErrorMessage = 'Аккаунт этого пользователя отключен.'; break;
        default: firebaseErrorMessage = 'Произошла непредвиденная ошибка. Попробуйте снова.';
      }
    ...
    <div className="login-page">
      ...
      <main className="login-container">
        <div className="login-form-wrapper">
          ...
          <p className="login-subtitle">Нет аккаунта? <Link to="/register" className="register-link">Зарегистрируйтесь</Link></p>
